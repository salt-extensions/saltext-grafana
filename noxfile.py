...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/grafana/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/grafana/*")
            ...
            #    "--include=src/saltext/grafana/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/grafana/config/schemas",
    )
